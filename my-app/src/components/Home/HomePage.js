  ...
  const PremiumAccordion = ({ title, icon, children, isOpen, onToggle }) => {
    return (
      <div className="border-b border-gray-100 last:border-0">
        <button
          onClick={onToggle}
          className="w-full px-4 sm:px-6 py-3 sm:py-4 flex justify-between items-center bg-white transition-all duration-200 group"
        >
  ...
  const MetalColorButton = ({ color, label, isSelected, onClick }) => {
    ...
    return (
      <button
        onClick={onClick}
        className={`relative px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg font-medium transition-all duration-200 text-sm sm:text-base ${
          isSelected 
      ...
      <div className="border-b border-gray-200 px-6 pt-4">
        <div className="flex gap-4 sm:gap-8">
          ...
          <button
            ...
            className={`pb-3 text-base sm:text-lg font-medium ${
              activeTab === "theme" ? "text-white border-b-2 border-white" : "text-gray-500"
            }`}
          >
            Theme
          </button>
  ...
  export default function HomePage() {
    ...
    const themes = [
      ...
      "bg-[radial-gradient(at_center,#f8fafc_0%,#f1f5f9_50%,#e2e8f0_100%)]",
      "bg-gradient-to-br from-zinc-200 via-slate-100 to-white",
                        ...
                        <div className="flex flex-wrap gap-1.5 sm:gap-2">
                          {visibleProngs.map((p) => (
                            <button
                              ...
                              className={`px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg transition-all text-xs sm:text-sm ${
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 gap-2 sm:gap-3">
                            {diamondOptions.map((d) => (
                              ...
                              >
                                ...
                                <span className="text-xs mt-1 block">{d.name}</span>
                              </button>
                            ))}
                          </div>
                        </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:gap-3">
                    {themes.map((theme, i) => (
                      <button
                        ...
                        // className="h-12 sm:h-16 rounded-lg transition-transform hover:scale-105"
                        style={{ background: theme.includes('bg-') ? undefined : theme }}
                        className={`h-12 sm:h-16 rounded-lg ${theme} transition-transform hover:scale-105`}
                      />
                    ))}
                  </div>
                )}
